...
from .serializers import ProgramSerializer, ActivitySerializer,ServiceSerializer
from .models import Program, Service
...
class RecentVerifiedAboutServicesView(ListAPIView):
    serializer_class = AboutSerializer

    def get_queryset(self):
        content_type = ContentType.objects.get_for_model(Service)  # Replace 'YourRelatedModel' with your specific model
        return About.objects.filter(
            is_verified=True,
            content_type=content_type,
            #object_id=your_object_id  # Replace 'your_object_id' with the ID of the related object
